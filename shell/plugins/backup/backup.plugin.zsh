...
backup_pc() {
    export BACKUPS_DIR=$BACKUPS_HOME/pc
...
backup_pc_file() {
    ...
    then
        ...
        excludes=$2
        excludes=$DOTFILES_HOME/shell/plugins/backup/$excludes
        rs --exclude-from=$excludes $file $BACKUPS_DIR$file
    fi
}

backup_server() {
    export BACKUPS_DIR=$BACKUPS_HOME/server
...
backup_router_device() {
    device=$1
    export BACKUPS_DIR=$BACKUPS_HOME/router
